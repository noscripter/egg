...
如果你的团队遇过：
1. 维护很多个项目，每个项目都需要复制拷贝诸如 `gulpfile.js` / `webpack.config.js` 之类的文件。
1. 每个项目都需要使用一些相同的类库，相同的配置。
1. 在新项目中对上面的配置做了一个优化后，如何同步到其他项目？
...
为此，egg 为团队架构师和技术负责人提供 `框架机制`，框架是一层抽象，可以基于 egg 去封装上层框架，并且 egg 支持多层继承。
...
你可以直接通过 [egg-bin] 选择 framework 脚手架来快速上手。
